...
| 28 | **bend26 = dokumen BULANAN per pos (4 jenis: Servis, Suku Cadang, Pelumas, Servis AC)** berformat **BUKTI KAS PENGELUARAN** sesuai file contoh kantor — digenerate on demand dari Realisasi Bulanan (agregasi seluruh nota bulan tsb; PPN = DPP×11%, PPh per tarif pos; identitas pejabat/NIP/No. Rek diatur admin). Tidak lagi otomatis per input nota. *(rev UAT 04-B6)* |
