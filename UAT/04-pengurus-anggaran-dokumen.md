...
| B3b | **Verifikasi ulang B3 (implementasi selesai)**: ketik `1000000` pada nominal rincian | Tampil **1.000.000** live + subtotal/total ikut terhitung (diverifikasi browser oleh developer: ketik 1000000 → "1.000.000", subtotal Rp 1.000.000, total Rp 1.130.000) | ok | |
...
| B4b | **Verifikasi B4 (implementasi selesai)**: simpan nota → lihat tab Selesai | Redirect ke tab Selesai; **maintenance terbaru paling atas**; kartu menampilkan tombol **Edit Nota** (oranye) menggantikan Input Nota; edit nilai → simpan → menu Dokumen menampilkan badge **"Diperbarui {tgl jam}"** pada draft nota terkait (jumlah dokumen tidak bertambah) | revisi | ketika edit nota, tombol bawah nya bukan nya harus nya selesai edit? bukan simpan nota & generate dokumen seperti ketika input nota? selain itu sudah ok|
| B4c | **Verifikasi B4b (implementasi selesai)**: buka form melalui tombol **Edit Nota** | Judul form **"Edit Nota Bengkel"**; tombol bawah **oranye "Selesai Edit"** (bukan "Simpan Nota & Generate Dokumen" — label itu hanya saat input pertama, hijau) | ok | |
...
| B6b | **Verifikasi B6 (implementasi selesai — format dari docs/Bend26/, PDF)**: Realisasi Bulanan → bulan ber-nota → **Generate Bend26 {bulan}** | 1 PDF per pos bernilai, berisi: Terima dari instansi, Uang sebesar + terbilang huruf, "Yaitu untuk pembayaran Belanja {pos} ... {daftar plat} ... Bulan {bulan}, Nota terlampir", kota + bulan kanan atas, ttd Pengguna Anggaran/Bendahara/Yang menerima (nama+NIP dari pengaturan), strip bawah: PPN = DPP×11%, PPh sesuai tarif pos, Jml, No. Rek per pos, Kode Kegiatan, Tahun Anggaran; regenerasi bulan sama = **timpa** (badge Diperbarui, tidak dobel) | revisi | terkait dokumen bend26 secara bentuk saya sangat cocok dengan draft yang kamu bikin pada bend26 servis, revisi sedikit pada dokumen nya adalah pada text : "Mengetahui dan menyetujui" untuk spasi atas dan bawah dari tulisan tersebut dihilangkan, kemudian apply ke bend26 pelumas, bend26 suku cadang, dan bend26 servis ac |
| B6d | **Verifikasi B6b (implementasi selesai)**: regenerate bend26 4 pos → buka PDF | Teks **"Mengetahui dan menyetujui"** menempel langsung di atas "Pengguna Anggaran," — spasi atas/bawah dihilangkan di semua 4 pos | revisi | terkait tulisan "Yogyakarta, September 2026" untuk bend26 pelumas dan servis sudah oke, namun untuk bend26 suku cadang dan bend26 servis ac, tulisan tersebut masih jadi 3 baris dan juga Mengetahui dan menyetujui masih ada space di atas dan bawah nya |
| B6e | **Verifikasi B6d (penyebab: file lama, bukan template — sudah ditimpa ulang)**: unduh ulang bend26 suku cadang & servis AC | Keempat pos kini seragam: "Yogyakarta, September 2026" **satu baris** + "Mengetahui dan menyetujui" tanpa spasi. Catatan: dokumen lama memakai layout saat digenerate — setelah revisi template, dokumen bulan berjalan perlu **di-generate ulang** (tombol Generate Bend26 → timpa di tempat) | ok | |
| B6c | Admin → Pengaturan Aplikasi → buka **Identitas Dokumen Bend26 (lanjutan)** | Semua identitas bisa diubah via UI (instansi, kota, 4 pejabat+NIP, kode kegiatan, PPN%, No. Rek & PPh% per pos) — satu tombol Simpan Pengaturan menyimpan semuanya | revisi | untuk ppn dan pph untuk bisa dikumpulkan untuk masing-masing pos anggaran, jadi no rek servis kemudian ppn servis dan pph servis, dan seterusnya, pisahkan ppn nya karena untuk pelumas tidak dikenakan ppn, namun suatu saat belum tentu pelumas tidak kena pajak makanya rumah nya kita siapkan|
...
| B7b | **Verifikasi B7 (implementasi selesai)**: unduh draft nota pos dengan ≥2 rincian | Tabel memuat **1 baris per rincian** (deskripsi + nominal asli) + footer JUMLAH = total nilai asli pos (belum koefisien) | ok | |
...
| B9b | **Verifikasi B9 (implementasi selesai)** | Tombol **Edit Nota** membuka form terisi rincian lama; simpan perubahan → tidak ada dokumen/record baru (timpa di tempat) | ok | |
...
| B10b | **Verifikasi B10 (implementasi selesai)**: menu Laporan → pilih mobil → Generate | PDF sesuai format; menu Dokumen & halaman anggaran kendaraan **tidak lagi** punya tombol kartu; menu Dokumen menampilkan label "Kartu Pemeliharaan" | ok | |
...
| B11b | **Verifikasi B11 (implementasi selesai)**: cek menu Dokumen setelah beberapa kali edit nota & regenerate bend26 | Tidak ada v2/v3; setiap dokumen satu record; badge Diperbarui memantulkan waktu perubahan terakhir | ok | |
| B12 | Buka form nota dari **ponsel** (viewport kecil / devtools mobile) | Form **mobile-first**: pos vertikal, tombol + rincian lebar, nominal berpemisah ribuan, tombol ≥44px nyaman disentuh | revisi | tampilan paling bawah masih terlihat tulisan setengah nya alias tertutupi oleh bar yang berisi Beranda, Cari Mobil, Pinjaman, Profil, klau bisa dinaikkan sedikit diatas bar tersebutnsehingga tulisan di paling bawah dapat terlihat utuh |
| B12b | **Verifikasi B12 (implementasi selesai)**: mobile → scroll ke paling bawah form nota | Konten terakhir (tombol Simpan/Batal) terlihat **utuh di atas** bar navigasi bawah (padding bawah mobile dinaikkan 112px → 144px) | ok | |
| B12c | **Verifikasi B12 di SEMUA halaman (app-shell — rev terakhir)**: buka berbagai halaman mobile (dashboard, riwayat, dokumen, laporan, dst) → scroll sampai bawah | Bar menu bawah **menjadi bagian halaman** (bukan lapisan menimpa): konten terakhir SELALU utuh 16px di atas bar di semua halaman — konten tidak pernah lewat/tertutup bar saat scroll | ok | |
...
| D1b | **Verifikasi D1 (implementasi selesai)** | List mobil **terbaru di atas** (urutan berdasar tanggal nota/maintenance terakhir tiap mobil); rincian nota dalam drill-down juga terbaru dulu; tombol **Generate Bend26 {bulan}** tampil saat ada data | ok | |
...
| E2b | **Verifikasi E2 (implementasi selesai)**: buka Pengaturan Aplikasi | **SATU tombol "Simpan Pengaturan" di paling bawah** form (tombol "Simpan Semua" & tombol ganda lama dihilangkan) — menyimpan durasi + koefisien + identitas bend26 sekaligus; terdapat seksi lanjutan "Identitas Dokumen Bend26" | ok | |
