...
| D8 | **Baru (pasca-03)**: buat booking MASA DEPAN (mis. H+7..H+8) → lihat kartunya | Tombol yang tampil **"Batalkan Peminjaman"** (merah) — BUKAN Selesai | ok | |
| D9 | **Baru**: tekan Batalkan Peminjaman → konfirmasi "Ya, Batalkan" | Status **Dibatalkan** + chip **"Dibatalkan {tanggal & jam}"** (bukan dikembalikan); kuota lepas (bisa booking lagi); mobil langsung bisa dipinjam di rentang itu | ok | |
| D10 | **Baru**: coba tekan Selesai pada booking belum mulai / Batalkan pada booking hari ini | Keduanya **ditolak** dengan pesan penjelas (guard silang) | ok | perjelas maksud uat ini, dalam pengecekan mobil yang dipinjam pada tanggal sekarang di riwayat tulisan nya selesai -- kembalikan mobil, dan untuk skenario peminjaman seperti itu sudah betul, kemudian bagaimana dengan skenario peminjaman Mobil A selama 3 hari dan sudah berjalan 2 hari lalu menekan tombol selesai, bagaimana status nya riwayat peminjaman, lalu apakah muncul di pegawai lain ketika hari terakhir ternyata tidak terpakai |
| D11 | **Jawaban D10 (implementasi + test teruji)**: booking Mobil A **kemarin..besok** (3 hari) → hari ini tekan Selesai | Status riwayat: **Dikembalikan** + waktu aktual (rentang asli kemarin..besar tetap tertera). ★ Mobil **LANGSUNG tersedia** untuk hari TERAKHIR (besok) — pegawai lain dapat langsung booking hari itu (teruji otomatis) | ok | |
...
| F4 | **Verifikasi ulang F1 (jam WIB)**: buat booking H..H → Selesai → cek jam di "Dikembalikan …" | Jam yang tampil = jam WIB saat menekan Selesai (perbaikan timezone Asia/Jakarta — data LAMA sebelum revisi tampil mundur 7 jam, hanya data baru yang acuan) | ok | |
| F5 | **Verifikasi ulang F2 (penggantian)**: ulangi skenario 03-C1 | Membuat maintenance menabrak booking kini **langsung mengarahkan ke halaman Penggantian Mobil** dengan peringatan kuning; banner "N menunggu mobil pengganti" tampil di Jadwal Maintenance | ok | |
| F6 | **Urutan riwayat (baru)**: pegawai punya booking AKTIF 14–15 Sep + booking DIBATALKAN 22–24 Sep → buka Peminjaman Saya | Yang **AKTIF tampil paling atas** (terdekat dulu), riwayat non-aktif di bawahnya terbaru dulu | ok | |
| F7 | **Pagination + filter bulan (revisi)**: buka Peminjaman Saya | **Default hanya bulan berjalan** (dropdown "Menampilkan riwayat: {bulan}" — pilih bulan lain untuk melihat data lama); peminjaman AKTIF selalu tampil; halaman 1 **tanpa tombol Sebelumnya**, halaman terakhir **tanpa tombol Berikutnya** | ok | |
