<x-app-layout title="{{ $user->exists ? 'Ubah User' : 'Tambah User' }}">
    ...
    <form method="POST"
        ...
        <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-2">
            <a href="{{ route('admin.users.index') }}" class="px-4 py-2 rounded-lg border border-gray-300 text-sm font-medium text-center hover:bg-gray-50 min-h-[44px] leading-[44px]">Batal</a>
            <x-primary-button>{{ $user->exists ? 'Simpan Perubahan' : 'Buat User' }}</x-primary-button>
        </div>
    </form>
</x-app-layout>
